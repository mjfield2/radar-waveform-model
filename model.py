...
def TM_model2d_custom(ep, mu, sig, xprop, zprop, srcloc, recloc, srcpulse, t, npml, outstep=1, plotopt=[1, 50, 0.05], quiet=False):
    if len(plotopt) == 2:
        plotopt.append(0.05)
...
def TM_model2d_custom_mp(ep, mu, sig, xprop, zprop, srcloc, recloc, srcpulse, t, npml, outstep=1, plotopt=[1, 50, 0.05], quiet=False):
    if __name__ == '__main__':
        ...
        for i in range(nsrc):
            src_i = srcloc[i,:]
            param_list.append([ep, mu, sig, xprop, zprop, src_i, recloc, srcpulse, t, npml, outstep, plotopt, True])
...
def jit_caller(ep, mu, sig, xprop, zprop, srcloc, recloc, srcpulse, t, npml, outstep=1, plotopt=[1, 50, 0.05]):
    if len(plotopt) == 2:
        plotopt.append(0.05)
    ...
    return TM_model2d_jit(ep, mu, sig, xprop, zprop, srcloc, recloc, srcpulse, t, npml, outstep, plotopt)
...
@jit(parallel=True)
def TM_model2d_jit(ep, mu, sig, xprop, zprop, srcloc, recloc, srcpulse, t, npml, outstep=1, plotopt=[1, 50, 0.05]):
    ep0 = 8.8541878176e-12
    mu0 = 1.2566370614e-6
    ep = ep * ep0
    mu = mu * mu0
    ...
    wav_cache = np.zeros((nsrc, len(tout), nz-1, nx-1))

    for s in prange(nsrc):
        iii = srci[s]
        jjj = srcj[s]
        #print(f'index: {(iii, jjj)}')
        print(f'source #: {s+1}')
        print(f'source position: x = {srcx[s]}, z = {srcz[s]}')
        # zero all field matrices
        Ey = np.zeros((nx - 1, nz - 1))  # Ey component of electric field
        Hx = np.zeros((nx - 1, nz))  # Hx component of magnetic field
        Hz = np.zeros((nx, nz - 1))  # Hz component of magnetic field
